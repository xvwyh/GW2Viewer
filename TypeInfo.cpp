...
TypeInfo::Enum* TypeInfo::Symbol::GetEnum()
{
    if (!Enum)
        return nullptr;
    if (!Enum->Name.empty())
        if (auto const itr = g_config.SharedEnums.find(Enum->Name); itr != g_config.SharedEnums.end())
            return &itr->second;
    return &*Enum;
}
...
TypeInfo::StructLayout& TypeInfo::Symbol::GetElementLayout()
{
    if (!ElementTypeName.empty())
        if (auto const itr = g_config.SharedTypes.find(ElementTypeName); itr != g_config.SharedTypes.end())
            return itr->second;
...
void TypeInfo::Symbol::DrawOptions(TypeInfo& typeInfo, LayoutStack const& layoutStack, std::string_view parentPath, bool create, std::string const& placeholderName)
{
    ...
    // Type
    {
        ...
        int type = std::distance(types, std::ranges::find(types, GetType()));
    ...
    // ElementTypeName
    {
        if (!ElementTypeShared)
            ElementTypeShared = !ElementTypeName.empty() && g_config.SharedTypes.contains(ElementTypeName);
        if (I::Checkbox("Shared", &*ElementTypeShared) && !ElementTypeName.empty())
        {
            if (*ElementTypeShared && !g_config.SharedTypes.contains(ElementTypeName))
                g_config.SharedTypes.emplace(ElementTypeName, std::exchange(ElementLayout, StructLayout()));
            else if (g_config.SharedTypes.contains(ElementTypeName))
                ElementLayout = g_config.SharedTypes.at(std::exchange(ElementTypeName, std::string()));
        }
        I::SameLine();
        if (*ElementTypeShared)
        {
            if (auto items = [this](int index) { return index >= 0 && index < g_config.SharedTypes.size() ? std::next(g_config.SharedTypes.begin(), index)->first.c_str() : "<c=#4>Anonymous Inline Type</c>"; };
                scoped::Combo("ElementTypeName", items(ElementTypeName.empty() ? -1 : std::distance(g_config.SharedTypes.begin(), g_config.SharedTypes.find(ElementTypeName))), ImGuiComboFlags_HeightLarge))
            {
                if (I::ComboItem(items(-1), ElementTypeName.empty()))
                    ElementTypeName.clear();
                for (auto const& [index, value] : g_config.SharedTypes | std::views::keys | std::views::enumerate)
    ...
    optionalToggle(Enum, ICON_FA_LIST_OL, "Enum", "Display as Enum", [&]
    {
        {
            if (!EnumShared)
                EnumShared = !Enum->Name.empty() && g_config.SharedEnums.contains(Enum->Name);
            if (I::Checkbox("Shared", &*EnumShared) && !Enum->Name.empty())
            {
                if (*EnumShared && !g_config.SharedEnums.contains(Enum->Name))
                    g_config.SharedEnums.emplace(Enum->Name, std::exchange(*Enum, TypeInfo::Enum { .Name = Enum->Name }));
                else
                {
                    if (g_config.SharedEnums.contains(Enum->Name))
                        *Enum = g_config.SharedEnums.at(Enum->Name);
                    Enum->Name.clear();
                }
            }
            I::SameLine();
            if (*EnumShared)
            {
                if (auto items = [this](int index) { return index >= 0 && index < g_config.SharedEnums.size() ? std::next(g_config.SharedEnums.begin(), index)->first.c_str() : "<c=#4>Anonymous Enum</c>"; };
                    scoped::Combo("##Name", items(Enum->Name.empty() ? -1 : std::distance(g_config.SharedEnums.begin(), g_config.SharedEnums.find(Enum->Name))), ImGuiComboFlags_HeightLarge))
                {
                    if (I::ComboItem(items(-1), Enum->Name.empty()))
                        Enum->Name.clear();
                    for (auto const& [index, value] : g_config.SharedEnums | std::views::keys | std::views::enumerate)
...
void TypeInfo::Symbol::Draw(byte const* data, DrawType draw, ContentObject& content)
{
    ...
    if (auto const contentPtr = type->GetContent(data))
    {
        if (auto const content = *contentPtr)
            replace_all(typeName, "Content", to_utf8(content->Type->GetDisplayName()));
        else if (!ElementTypeName.empty())
            replace_all(typeName, "T", ElementTypeName);
    }
    else if (!ElementTypeName.empty())
        replace_all(typeName, "T", ElementTypeName);
    ...
    switch (draw)
    {
        ...
        case DrawType::Inline:
            for (auto const part : { g_config.ShowContentSymbolNameBeforeType, !g_config.ShowContentSymbolNameBeforeType })
            {
                I::AlignTextToFramePadding();
                switch (part)
                {
...
void to_json(nlohmann::ordered_json& json, TypeInfo::StructLayout const& self)
{
...
void from_json(nlohmann::ordered_json const& json, TypeInfo::StructLayout& self)
{
