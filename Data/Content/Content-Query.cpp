...
concept SymbolDataSearcher = requires(T const a, TypeInfo::Symbol& symbol, byte const* data, ContentObject const& relative)
{
    ...
    { a.CanSearchDeeper() } -> std::same_as<bool>;
...
QuerySymbolDataResult::Generator QuerySymbolDataImpl(TypeInfo::LayoutStack& layoutStack, std::span<byte const> fullData, Searcher searcher)
{
    ...
    bool const deeper = searcher.CanSearchDeeper();
...
QuerySymbolDataResult::Generator QuerySymbolData(ContentObject const& content, SymbolPath::Span path)
{
    struct PathSearcher
    {
        ...
        [[nodiscard]] bool CanSearchDeeper() const { return Path.size() > 1; }
...
QuerySymbolDataResult::Generator QuerySymbolData(ContentObject const& content, TypeInfo::SymbolType const& type, TypeInfo::Condition::ValueType value)
{
    struct TypeSearcher
    {
        ...
        [[nodiscard]] bool CanSearchDeeper() const { return true; }
...
{
    static auto const guidSymbol = Symbols::GetByName("GUID");
    struct Exporter
    {
        ContentObject const& Content;
        ExportOptions const& Options;
        ordered_json* Output = nullptr;
        uint32 Depth = 0;
        mutable TypeInfo::Symbol* LastSymbol = nullptr;
        mutable byte const* LastData = nullptr;
        mutable ordered_json* LastOutput = nullptr;
        [[nodiscard]] SymbolPath::Part Root() const { return { }; }
        [[nodiscard]] bool CanSearch() const { return Output; }
        [[nodiscard]] bool CanCheck(TypeInfo::Symbol& symbol) const { return true; }
        [[nodiscard]] bool CanReturn(TypeInfo::Symbol& symbol, byte const* data) const
        {
            LastSymbol = &symbol;
            LastData = data;
            LastOutput = nullptr;

            if (Options.IgnoreUnnamedFields && symbol.Name.starts_with("field"))
                return false;

            auto const type = symbol.GetType();
            if (auto const object = type->GetContent({ data, Content, symbol }); object
                && !(type == guidSymbol && symbol.Autogenerated) // Skip Content::GUID pointing to itself
                && !type->IsArray() // Allow SymbolType::Export to return the json array
                && (type->IsInline() || CanStepIntoNonInlineContent())) // Only step into non-inlined content objects if we're below max depth
            {
                if (*object)
                    LastOutput = &((*Output)[symbol.Name] = ordered_json::object());
                else
                    LastOutput = &((*Output)[symbol.Name] = nullptr);
            }
            else
                LastOutput = &((*Output)[symbol.Name] = type->Export({ data, Content, symbol }, Options));
        ...
        [[nodiscard]] bool CanBacktrack() const { return false; }
        [[nodiscard]] bool CanBacktrackFrom(byte const* data) const { return false; }
        [[nodiscard]] bool CanStepIntoNonInlineContent() const { return Depth < Options.ContentPointerMaxInlineDepth; }
        [[nodiscard]] bool CanSearchDeeper() const { return true; }
        [[nodiscard]] Exporter Deeper() const
        {
            auto const type = LastSymbol->GetType();
            return {
                Content,
                Options,
                LastOutput && type->IsArray()
                ? &LastOutput->emplace_back(ordered_json::object()) // Add a new element to the array to write nested array element data to
                : LastOutput, // Write nested data to the last added element
                type->IsContent() ? Depth + 1 : Depth,
            };
        }
        [[nodiscard]] Exporter Deeper(ContentObject const& relative) const { std::terminate(); }
    };
    ordered_json json;
    QuerySymbolDataImpl(content, Exporter { content, options, &json }).begin();
    return json;
}
