...
ordered_json ExportSymbolData(ContentObject const& content, ExportOptions const& options)
{
    ...
    struct Exporter
    {
        ...
        [[nodiscard]] bool CanReturn(TypeInfo::Symbol& symbol, byte const* data) const
        {
            ...
            {
                if (*object)
                    Output[symbol.Name] = ordered_json::object();
                else
                    Output[symbol.Name] = nullptr;
            }
            else
                Output[symbol.Name] = type->Export({ data, Content, symbol });
            return false;
        }
        [[nodiscard]] bool CanEarlyReturn() const { return false; }
        [[nodiscard]] bool CanBacktrack() const { return false; }
        [[nodiscard]] bool CanBacktrackFrom(byte const* data) const { return false; }
        [[nodiscard]] bool CanStepIntoNonInlineContent() const { return Depth < Options.InlineObjectMaxDepth; }
        [[nodiscard]] bool CanSearchDeeper() const { return true; }
        [[nodiscard]] Exporter Deeper() const
        {
            ...
            return {
                Content,
                Options,
                type->IsArray() ? Output.back().emplace_back(ordered_json::object()) : Output.back(),
                type->IsContent() ? Depth + 1 : Depth,
            };
        }
        [[nodiscard]] Exporter Deeper(ContentObject const& relative) const { return Deeper(); }
    };
    for (auto& result : QuerySymbolDataImpl(content, Exporter { content, options, json }))
        ;
    return json;
}
