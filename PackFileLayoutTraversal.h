...
template<typename T = FieldIterator>
FieldIterator::Generator<T> QueryPackFileFields(FieldIterator::Bounds bounds, std::string_view path)
{
    struct PathPart
    {
    ...
    for (auto const& part : std::views::split(path, std::string_view(".")))
    {
        ...
        if (auto const subscriptIndex = name.find('['); subscriptIndex != std::string_view::npos)
        {
            ...
            if (subscriptEndIndex == std::string_view::npos)
                throw std::exception("QueryPackFileFields() called with a malformed path parameter: array subscript not closed");
            ...
            else if (auto result = scn::scan_value<uint32>(index))
                parts.emplace_back(name, true, result.value());
            else
                throw std::exception("QueryPackFileFields() called with a malformed path parameter: failed to parse array index");
        }
        else
...
template<typename T = FieldIterator>
FieldIterator::Generator<T> QueryPackFileFields(PackFile const& file, PackFileChunk const& chunk, std::string_view path)
{
    if (auto const itrChunk = g_chunks.find(std::string_view { (char const*)&chunk.Header.Magic, 4 }); itrChunk != g_chunks.end())
        if (auto const itrChunkVersion = itrChunk->second.find(chunk.Header.Version); itrChunkVersion != itrChunk->second.end())
            for (auto&& result : QueryPackFileFields<T>(FieldIterator::MakeFieldIteratorBounds(chunk.Data, file.Header.Is64Bit, *itrChunkVersion->second), path))
                co_yield result;
}

template<typename T = FieldIterator>
FieldIterator::Generator<T> QueryPackFileFields(PackFile const& file, fcc chunkMagic, std::string_view path)
{
    for (auto const& chunk : file)
        if (chunk.Header.Magic == chunkMagic)
            for (auto&& result : QueryPackFileFields<T>(file, chunk, path))
                co_yield result;
}
...
{
    for (auto&& result : QueryPackFileFields<T>(GetTargetFields(), path))
        co_yield result;
}
...
inline pf::Layout::Traversal::QueryChunk pf::PackFile::QueryChunk(fcc magic) const
{
    return { *this, GetChunk(magic) };
}
