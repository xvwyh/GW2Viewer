...
STATIC(MapLayoutViewer::ObjectSettings::Default);
...
void MapLayoutViewer::AddBackdrop(float scale, uint32 unexploredFileID, uint32 exploredFileID, bool water, bool interior)
{
    if (unexploredFileID)
        UnexploredBackdrops.emplace_back(g_archives.GetPackFile(unexploredFileID), scale);
    if (exploredFileID)
        ExploredBackdrops.emplace_back(g_archives.GetPackFile(exploredFileID), scale, water, interior);
}

MapLayoutViewer::Icon& MapLayoutViewer::AddIcon(uint32 textureFileID, ContentObject& mapDef, ImVec2 mapPosition, ImVec2 size)
{
...
void MapLayoutViewer::Initialize()
{
...
void MapLayoutViewer::Draw()
{
    ...
    if (std::exchange(ExploredMaskDirty, false))
        ExploredMaskTexture = CreateTexture(exploredMaskSize, exploredMaskSize, ExploredMask.data());
...
float4 main(PS_INPUT input) : SV_Target
{
    ...
    if (std::fabs(ViewportScale - ViewportScaleChaseTarget) < 0.00001f)
        ...
        ViewportScale = ExpDecay(ViewportScale, ViewportScaleChaseTarget, 15.0f, g_ui.DeltaTime());
    ...
    for (auto const* backdrops : { &UnexploredBackdrops, &ExploredBackdrops })
    {
        ...
        {
            float alpha = std::clamp(Remap(1.0f / ViewportScale, backdrop.Scale, backdrop.Scale * 2, 1.0f, 0.0f), 0.0f, 1.0f);
                ...
                for (auto const& image : layer.Images)
                    if (viewportWorldRect.Overlaps(image.BoundingBox))
                        if (scoped::WithCursorScreenPos(ImRoundIf(project(image.BoundingBox.Min), ViewportScale == 1.0f)))
                        {
                            ...
                            DrawTexture(image.TextureFileID,
                            {
                                .Color = { 1, 1, 1, alpha },
    ...
    for (auto const& icon : Icons)
    {
        if (viewportWorldRect.Overlaps(icon.BoundingBox))
        {
            ...
            {
                DrawTexture(icon.TextureFileID, { .Size = icon.BoundingBox.GetSize() * scale, .FullPreviewOnHover = false, .AdvanceCursor = false });
                if (scoped::ItemTooltip(ImGuiHoveredFlags_DelayNone))
        ...
        if (scoped::Table("ObjectSettings", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_NoSavedSettings | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable | ImGuiTableFlags_SizingFixedFit))
        {
            ...
            auto renderTree = [this](ContentObject const* source, LayoutTreeNode& node, bool parentVisible, bool& makeParentVisible, auto& renderTree) -> void
            {
                ...
                if (scoped::WithID(id))
                {
                    ...
                    {
                        I::SameLine(0, 0);
                        DrawTexture(node.TextureFileID, { .Size = I::GetFrameSquare() });
                    }
                    I::SameLine(0, 0);
                    if (source)
                    {
                        DrawContentButtonOptions::CondenseContext condense { .FullName = true, .TypeName = true };
                        DrawContentButton((ContentObject*)source, id, { .SharedCondenseContext = &condense }); // TODO: Fix constness
                    }
                    else
                        I::Text("%s (%zu)", to_utf8(node.Name), node.Visible);
                ...
                if (open)
                {
                    ...
                    std::vector sorted { std::from_range, node.Children | std::views::transform(proj::addressof) };
