...
struct ValueRepNumeric : ValueRepBase
{
    ...
    void Draw(byte const* dataPtr, uint32 available) override
    {
        ...
        if constexpr (std::is_same_v<T, wchar_t>)
            I::TextUnformatted(std::format("{}", std::wstring_view((T const*)dataPtr, 1)).c_str());
        else
...
struct ValueRepString : ValueRepBase
{
    ...
    void Draw(byte const* dataPtr, uint32 available) override
    {
        ...
        {
            utf8 = to_utf8(std::wstring_view(start, chars));
...
struct ValueRepToken32 : ValueRepBase
{
    void Draw(byte const* dataPtr, uint32 available) override
    {
        if (available < sizeof(pf::Token32))
            return;

        auto const& token = *(pf::Token32 const*)dataPtr;
        if (token.empty())
            return;
...
struct ValueRepToken64 : ValueRepBase
{
    void Draw(byte const* dataPtr, uint32 available) override
    {
        if (available < sizeof(pf::Token64))
            return;

        auto const& token = *(pf::Token64 const*)dataPtr;
        if (token.empty())
            return;
...
struct ValueRepFileID : ValueRepBase
{
    void Draw(byte const* dataPtr, uint32 available) override
    {
        ...
        if (!g_archives.ContainsFile(fileID))
            return;
...
struct ValueRepFileReference : ValueRepBase
{
    void Draw(byte const* dataPtr, uint32 available) override
    {
        if (available < sizeof(pf::FileReference))
            return;

        auto const fileID = ((pf::FileReference const*)dataPtr)->GetFileID();
        if (!g_archives.ContainsFile(fileID))
            return;
...
struct ValueRepStringID : ValueRepBase
{
    void Draw(byte const* dataPtr, uint32 available) override
    {
        ...
        if (!stringID || stringID >= g_maxStringID)
            return;

        auto [string, status] = GetString(stringID);
        std::string const utf8 = string ? to_utf8(*string) : "";

        I::TableNextRow();
        I::TableNextColumn(); I::TextUnformatted("StringID");
        I::TableNextColumn(); I::Text("%u", stringID);
        I::TableNextColumn(); I::Text("%s%s", GetEncryptionStatusText(status), utf8.c_str());
    }
};
struct ValueRepContentObject : ValueRepBase
{
    void Draw(byte const* dataPtr, uint32 available) override
    {
        if (available < sizeof(ContentObject*))
            return;

        auto const contentDataPtr = *(byte* const*)dataPtr;
        auto const object = GetContentObjectByDataPointer(contentDataPtr);
        if (!object)
            return;
        object->Finalize();
        ...
        I::TableNextColumn(); I::Text("%s", to_utf8(object->Type->GetDisplayName()).c_str());
        I::TableNextColumn(); I::Text("%s", to_utf8(object->GetFullDisplayName()).c_str());
    }
};
