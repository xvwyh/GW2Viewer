...
namespace dear
{
    ...
    struct OverrideID : public ScopeWrapper<OverrideID>
    {
        template<typename... Args>
        OverrideID(Args&&... args) noexcept : ScopeWrapper(true)
        {
            ImGui::PushOverrideID(std::forward<Args>(args)...);
        }
        static void dtor() noexcept
        {
            ImGui::PopID();
	...
	struct Indent : public ScopeWrapper<Indent>
	{
        Indent(float width) noexcept : ScopeWrapper(true), m_width(width)
		{
            ImGui::Indent(m_width);
		}
		void dtor() noexcept
		{
