...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // https://github.com/ocornut/imgui/issues/6421#issuecomment-1712170404
    // For non-scrolling tables, we can't rely on GetContentRegionAvail() because it
    // returns the width of the window's client area.  That's why we use WorkRect instead.
    ImVec2 actual_outer_size = CalcItemSize(
            outer_size,
            ImMax(use_child_window? avail_size.x : outer_window->WorkRect.GetWidth(), 1.0f),
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX)
    {
        ...
        // https://github.com/ocornut/imgui/issues/6421#issuecomment-1712170404
        // The actual rightmost boundary of the outer rect matches that of the
        // rightmost column, without clipping (since clipping may occur due to
        // the table not fitting into window's client area).
        const float rightmost_x = ImMax(table->WorkRect.Min.x, table->Columns[table->RightMostEnabledColumn].MaxX);
        table->OuterRect.Max.x = table->WorkRect.Max.x = rightmost_x;
