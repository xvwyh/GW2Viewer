...
static void ImGui_ImplDX11_SetupRenderState(ImDrawData* draw_data, ID3D11DeviceContext* device_ctx)
{
    ...
    ImGui_ImplDX11_SetPixelShader(bd->pPixelShader);
    device_ctx->PSSetSamplers(0, 1, &bd->pFontSampler);
    ID3D11Buffer* nullbuffers[] = { nullptr };
    device_ctx->PSSetConstantBuffers(0, 1, nullbuffers);
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        D3D11_INPUT_ELEMENT_DESC local_layout[] =
        {
            ...
            { "TEXCOORD", 1, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)offsetof(ImDrawVert, uv2), D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { "COLOR",    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)offsetof(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0 },
        };
        if (bd->pd3dDevice->CreateInputLayout(local_layout, sizeof(local_layout) / sizeof(*local_layout), vertexShaderBlob->GetBufferPointer(), vertexShaderBlob->GetBufferSize(), &bd->pInputLayout) != S_OK)
        {
            vertexShaderBlob->Release();
            return false;
        }
    ...
    if (!(bd->pPixelShader = ImGui_ImplDX11_CompilePixelShader(
        "struct PS_INPUT\
            {\
            float4 pos : SV_POSITION;\
            float4 col : COLOR0;\
...
ID3D11PixelShader* ImGui_ImplDX11_CompilePixelShader(char const* code)
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    ...
    if (FAILED(D3DCompile(code, strlen(code), nullptr, nullptr, nullptr, "main", "ps_4_0", 0, 0, &pixelShaderBlob, &errorBlob)))
    {
        OutputDebugStringA((char const*)errorBlob->GetBufferPointer());
        return nullptr; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
    }
    if (bd->pd3dDevice->CreatePixelShader(pixelShaderBlob->GetBufferPointer(), pixelShaderBlob->GetBufferSize(), nullptr, &result) != S_OK)
    {
...
IMGUI_IMPL_API void ImGui_ImplDX11_SetPixelShader(ID3D11PixelShader* shader)
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();

    bd->pd3dDeviceContext->PSSetShader(shader, nullptr, 0);
}

ID3D11Buffer* ImGui_ImplDX11_CreateBuffer(size_t size)
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    ...
    desc.MiscFlags = 0;
    ID3D11Buffer* result;
    bd->pd3dDevice->CreateBuffer(&desc, nullptr, &result);
    return result;
}

void ImGui_ImplDX11_SetPixelShaderConstantBuffer(ID3D11Buffer* buffer, void const* data, size_t size)
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();

    D3D11_MAPPED_SUBRESOURCE mapped_resource;
    if (bd->pd3dDeviceContext->Map(buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) != S_OK)
        return;
    memcpy(mapped_resource.pData, data, size);
    bd->pd3dDeviceContext->Unmap(buffer, 0);
    bd->pd3dDeviceContext->PSSetConstantBuffers(0, 1, &buffer);
}

void ImGui_ImplDX11_SetPixelShaderShaderResource(int slot, void* resource)
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    ID3D11ShaderResourceView* texture_srv = (ID3D11ShaderResourceView*)resource;
    bd->pd3dDeviceContext->PSSetShaderResources(slot, 1, &texture_srv);
}
