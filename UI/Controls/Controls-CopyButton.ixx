...
void CopyButton(char const* name, std::wstring_view data, bool condition = true) { CopyButton(name, data.size() <= 20 ? Utils::Encoding::ToUTF8(data) : Utils::Encoding::ToUTF8(std::format(L"{}...", std::wstring_view(data).substr(0, 20))), condition); }
